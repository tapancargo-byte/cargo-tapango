...
function scanFile(file) {
  ...
  if (/\b(jest|vi)\.mock\(/.test(text)) {
    errors.push(
      `[forbidden mock] ${rel}: avoid jest/vi.mock in Storybook runtime`
    );
  }
  ...
  if ((ext === '.ts' || ext === '.js') && jsxTag.test(text)) {
    errors.push(
      `[jsx-in-non-jsx] ${rel}: JSX detected in ${ext} file (rename to .tsx/.jsx)`
    );
  }
  ...
  if (
    /\bReact\./.test(text) &&
    !/import\s+React\s+from\s+'react'/.test(text) &&
    !/import\s+\*\s+as\s+React\s+from\s+'react'/.test(text)
  ) {
    errors.push(
      `[react-without-import] ${rel}: uses React.* without importing React`
    );
  }

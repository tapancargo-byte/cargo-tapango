...
export const StatusPill: React.FC<{ status: 'in-transit' | 'delivered' | 'pending' | 'delayed'; delayMs?: number }>
  = ({ status, delayMs = 0 }) => {
  ...
  React.useEffect(() => {
    ...
    const start = () => Animated.parallel([
      Animated.timing(opacity, { toValue: 1, duration: 180, easing: Easing.out(Easing.ease), useNativeDriver: useDriver }),
      Animated.spring(scale, { toValue: 1, useNativeDriver: useDriver, friction: 6, tension: 140 })
    ]).start()
    if (delayMs > 0) {

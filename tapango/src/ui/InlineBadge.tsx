...
export const InlineBadge: React.FC<InlineBadgeProps> = ({ text, tone = 'info' }) => {
  ...
  const luminance = ({r,g,b}:{r:number;g:number;b:number}) => 0.2126*sRGB(r)+0.7152*sRGB(g)+0.0722*sRGB(b)
  const ratio = (fg:string,bg:string) => { const L1=luminance(toRgb(fg)); const L2=luminance(toRgb(bg)); const br=Math.max(L1,L2); const dk=Math.min(L1,L2); return (br+0.05)/(dk+0.05) }
  ...
  return (
    <YStack paddingHorizontal="$2" paddingVertical="$1" borderRadius="$2" backgroundColor={t.bg}>
      <Text fontSize={font.caption} fontWeight="700" color={finalFg} textTransform="uppercase">{text}</Text>
    </YStack>
  )
}

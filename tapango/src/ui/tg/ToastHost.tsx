...
export const AppToastProvider = ({ children }: PropsWithChildren) => {
  ...
  const playOut = useCallback((onEnd?: () => void) => {
    Animated.parallel([
      Animated.timing(opacity, { toValue: 0, duration: 160, easing: Easing.in(Easing.ease), useNativeDriver: true }),
      Animated.timing(translate, { toValue: 16, duration: 160, easing: Easing.in(Easing.ease), useNativeDriver: true }),
  ...
  const api = useMemo<ToastAPI>(() => ({
    show: (title: string, message?: string, opts?: { variant?: ToastVariant }) => {
      const item: ToastItem = { id: String(Date.now() + Math.random()), title, message, variant: opts?.variant ?? 'info' }
      setQueue((q) => [...q, item])
    },
    hide: () => {
      if (current) {
  ...
  return (
    <ToastCtx.Provider value={api}>
      {children}
      {current ? (
        <Animated.View pointerEvents="box-none" style={[styles.host, { bottom: insets.bottom + 16, opacity, transform: [{ translateY: translate }] }]}>
          <YStack elevation={4} backgroundColor="$background" borderRadius="$4" padding="$3" borderColor={colors.border} borderWidth={1} maxWidth={360} alignSelf="center">
            <XStack alignItems="center" space="$2">
              {/* Left accent icon based on variant */}
              {current.variant === 'success' && <Ionicons name="checkmark-circle" size={18} color="#10B981" />}
              {current.variant === 'warning' && <Ionicons name="warning" size={18} color="#F59E0B" />}
              {current.variant === 'error' && <Ionicons name="alert-circle" size={18} color="#EF4444" />}
              {current.variant === 'info' && <Ionicons name="information-circle" size={18} color="#60A5FA" />}
              <YStack>

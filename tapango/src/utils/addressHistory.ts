...
export async function addRecentAddress(kind: AddressKind, addr: AddressSelection) {
  try {
    const key = KEYS[kind];
    const raw = await AsyncStorage.getItem(key);
    const list: AddressSelection[] = raw ? JSON.parse(raw) : [];
    const deduped = [addr, ...list.filter((x) => x.formatted !== addr.formatted)];
...
export async function getRecentAddresses(kind: AddressKind): Promise<AddressSelection[]> {
  try {

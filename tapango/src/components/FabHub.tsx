...
const FabHub: React.FC = () => {
  ...
  return (
    <YStack position="absolute" bottom={bottom} left={0} right={0} alignItems="center" pointerEvents="box-none">
      {/* Fan actions */}
      {open && (
        <XStack space="$3" marginBottom="$3" pointerEvents="box-none">
          <FabAction label="Track" onClose={() => setOpen(false)} onPress={() => router.push('/tracking')} />
          <FabAction label="Orders" onClose={() => setOpen(false)} onPress={() => router.push('/orders')} />
          <FabAction label="Profile" onClose={() => setOpen(false)} onPress={() => router.push('/profile')} />
        </XStack>
      )}
      ...
      >
        <XStack alignItems="center" space="$2">
          ...
          <Text color="#fff" fontWeight="700">Book</Text>
        </XStack>
      </Button>
    </YStack>
  )

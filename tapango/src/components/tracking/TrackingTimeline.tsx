...
const TimelineItem = memo(function TimelineItem({ event, formatDateTime, isLast, index }: TimelineItemProps) {
  const palette = useColors();
...
const mapStatusToIcon = (
  ...
  switch (status) {
    case 'confirmed': {
      return { iconName: 'checkmark', color: palette.success };
    }
    case 'in-transit': {
      return { iconName: 'car', color: palette.primary };
    }
    case 'delayed': {
      return { iconName: 'warning', color: palette.warning };
    }
    case 'cancelled': {
      return { iconName: 'close', color: palette.error };
    }
    case 'pending': {
      return { iconName: 'time', color: palette.info };
    }
    case 'delivered': {
      return { iconName: 'checkmark-done', color: palette.success };
    }
    default: {
      return { iconName: 'ellipse', color: palette.border };
    }
  }
};

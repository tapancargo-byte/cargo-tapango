...
export default function DriverProfile() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data: { subscription } } = supabase.auth.onAuthStateChange((_evt: unknown, session: any) => {
  ...
  return (
    <View style={styles.container}>
      ...
      {!supabase && (
        <Text style={[styles.subtitle, { marginTop: 12 }]}>Supabase not configured. Set EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY.</Text>
      )}
        ...
        <View style={{ marginTop: 16 }}>
          {userId ? (
            ...
            <View>
              ...
              <Button variant="primary" onPress={async () => {
                ...
                try {
                  ...
                  if (error) Alert.alert('Error', error.message);
                    ...
                    try { await drainDriverOffers(); } catch {}
                    try { await drainKycUploads(); } catch {}
              ...
              <Button variant="secondary" onPress={() => router.push('/(driver)/sign-in' as any)} fullWidth style={{ marginTop: 8 }}>
                Open sign-in screen
              </Button>
            </View>
          )}

...
const HeaderBar = ({ title = 'Demo Header', subtitle = 'Sticky on scroll' }) => (
  <YStack padding="$4" paddingBottom="$3" backgroundColor="$background" borderBottomWidth={1} borderColor="$borderColor">
    <Text fontSize={20} fontWeight="700">{title}</Text>
    <Text color="$colorFocus" opacity={0.8}>{subtitle}</Text>
  </YStack>
)
...
export const ScrollWithStickyHeader: StoryObj<typeof Screen> = {
  render: (args) => (
    <Screen
      {...args}
      header={<HeaderBar title="Feed" subtitle="Sticky header + safe area" />}
    >
      <YStack space="$4">
  ...
  render: (args) => (
    <Screen {...args} header={<HeaderBar title="Static Screen" subtitle="No scrolling" />}>
      <YStack space="$4">
        ...
        <Stack height={160} backgroundColor="$color2" borderRadius="$4" alignItems="center" justifyContent="center">
